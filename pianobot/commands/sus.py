    ...
    async def sus(self, ctx: commands.Context, player: str):
        ...
        try:
            async with ClientSession() as session, session.get(
                ...
                uuid = response['uuid']
                first_name_change = datetime.strptime(
                    response['username_history'][1]['changed_at'],
                    '%Y-%m-%dT%H:%M:%S.%fZ'
                ) if len(response['username_history']) > 1 else None
                ashcon_created = datetime.strptime(
                    response['created_at'],
                    '%Y-%m-%d'
        ...
        scores = [
            ...
            min(
                ['PLAYER', None, 'VIP', 'VIP+', 'HERO', 'CHAMPION'].index(wynncraft_rank) * 25,
                100
            ),
