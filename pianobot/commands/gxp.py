...
class GXP(commands.Cog):
    ...
    @commands.command(
        ...
        help = 'This command returns a list with the amount of guild experience members of Eden'
            ...
            ' (e.g. `3`) after the command (the previous examples would form the command `xp d 3`'
    ...
    async def gxp(self, ctx: commands.Context, unit: str = '', interval: int = 1):
        ...
        if unit == '':
            ...
            if unit.startswith('-'):
                unit = unit[1:]
            unit = {'m': 'minute', 'h': 'hour', 'd': 'day', 'w': 'week'}.get(unit.lower(), None)
            if unit is None:
                await ctx.send('Not a valid time unit!')
                return

            oldest_data = self.bot.database.guild_xp.get_first(f'{interval} {unit}')
            newest_data = self.bot.database.guild_xp.get_last(2)
            if oldest_data is None or oldest_data.time == newest_data[0].time:
        ...
        differences = [
            [name, display(xp)] for name, xp in sorted(
...
def display(num: float) -> str:
    ...
    pos = max(0, min(len(names) - 1, int(math.floor(0 if num == 0 else math.log10(abs(num)) / 3))))
