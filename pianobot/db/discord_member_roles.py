...
class DiscordMemberRole:
    def __init__(self, member_id, role_id):
...
class DiscordMemberRoleTable:
    ...
    def get_all(self, member_id: int) -> list[DiscordMemberRole]:
        result = self._con.query(
            'SELECT * FROM discord_member_roles WHERE member_id = %s;',
            member_id
        )
        return [DiscordMemberRole(row[0], row[1]) for row in result]

    def add(self, member_id: int, role_id: int):
    ...
    def remove(self, member_id: int, role_id: int):
        self._con.query(
